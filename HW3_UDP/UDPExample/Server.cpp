...
bool CheckIsOff(string strMessage)
{
	string strClientMessage = strMessage;
	string checkExitSubstr;
	size_t pos = strClientMessage.find(' ');
	if (pos != string::npos)
	{
		string temp = strClientMessage.substr(pos + 1);
		checkExitSubstr = temp.substr(0, 3);
	}
	if (checkExitSubstr == "off")
	{
		return true;
	}
	else
...
int main() {
	...
	while (true) {
		...
		if (FD_ISSET(server_socket, &readfds)) {
			...
			string newConnectionMessage = "\nNew connection, socket fd is " + to_string(new_socket) + ", ip is: " + string(inet_ntoa(address.sin_addr)) + ", port: " + to_string(ntohs(address.sin_port)) + "\n";
			cout << newConnectionMessage;
			history.push_back(newConnectionMessage);

			for (int i = 0; i < MAX_CLIENTS; i++) {
				if (client_socket[i] != 0) {
					send(client_socket[i], newConnectionMessage.c_str(), newConnectionMessage.size(), 0);
				}
			}
		...
		for (int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if (FD_ISSET(s, &readfds))
			{
				...
				string strClientMessage = client_message;
				if (CheckIsOff(client_message))
				{
					string offMessage = "\nClient #" + to_string(i) + " is off\n";
					cout << offMessage;
					client_socket[i] = 0;
					...
					for (int j = 0; j < MAX_CLIENTS; j++)
					{
						if (client_socket[j] != 0) {
							send(client_socket[j], offMessage.c_str(), offMessage.size(), 0);
						}
					}
				}
				...
				for (int i = 0; i < MAX_CLIENTS; i++) {
					if (client_socket[i] != 0 && CheckIsOff(client_message) == false)
					{
