...
int main()
{
    ...
    vector<string> keys = { "dt", "country", "name", "lon", "lat", "temp", "sunrise", "sunset"};
    ...
    for (int i = 0; i < keys.size(); i++)
    {
        ...
        if (found != string::npos)
        {
            ...
            if (keys[i] == "sunrise"|| keys[i] == "sunset"|| keys[i] == "dt")
            {
                int number = stoi(extracted);
                time_t  time = number;
                char* dt = ctime(&time);
                tm* gmtm = gmtime(&time);
                dt = asctime(gmtm);
            ...
            KeyValue.insert({ keys[i], extracted});
        }
    }
    cout << "Map data:" << endl;
    for (const auto& elem : KeyValue)
    {
